...
本ソフトウェアはプロプライエタリなフリーウェア（オープンソースではありません）であり、[LICENSE](./LICENSE) のエンドユーザーライセンス契約に基づいて提供されます。無償でダウンロードし、個人的かつ非商用目的で利用できます。改変、リバースエンジニアリング、再配布、および無断での商用利用は禁止されています。
